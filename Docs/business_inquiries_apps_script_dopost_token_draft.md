...
가장 단순한 1차안은 아래 방식입니다.

- 프론트가 `requestToken` 값을 같이 보낸다.
- Apps Script가 `EXPECTED_REQUEST_TOKEN`과 비교한다.
- 같으면 저장
- 다르면 거부
...
```text
requestToken=HAGOBOGO_INQUIRY_2026_SECRET
```
...
const EXPECTED_REQUEST_TOKEN = 'HAGOBOGO_INQUIRY_2026_SECRET';
...
### 6-2. `EXPECTED_REQUEST_TOKEN`

```javascript
const EXPECTED_REQUEST_TOKEN = 'HAGOBOGO_INQUIRY_2026_SECRET';
```

이 값은 대장님이 직접 정한 토큰으로 바꿔야 합니다.
...
1. `SHEET_NAME` 수정 완료
2. `EXPECTED_REQUEST_TOKEN` 수정 완료
