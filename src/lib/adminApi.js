import { ADMIN_PASSWORD } from '../utils/adminSettings';
...
async function postAdminRequest(payload) {
    ...
    if (!inquiryApiUrl) {
        const error = new Error('관리자 설정용 Apps Script URL이 설정되지 않았습니다.');
...
export async function fetchAdminNotificationEmail() {
    const data = await postAdminRequest({
        action: 'GET_NOTIFICATION_EMAIL',
    });

    return data.notificationEmail || '';
}

export async function saveAdminNotificationEmail(notificationEmail) {
    const data = await postAdminRequest({
        action: 'SET_NOTIFICATION_EMAIL',
        notificationEmail,
    });

    return data.notificationEmail || notificationEmail;
}
